...
function createFlow(iso, flows, direction) {
    ...
    if (direction == "out") {
        ...
        for (var j = 0; j < flows.length; j++) {
            iso = flows[j].ans_owner_country;
            d3.select("#countryLabel" + iso).style("display", "block");
            if (iso != flows[j].ques_owner_country) {
                d3.select("#countryLabel" + iso).select('text').attr('class', 'countryNameSourceFlow');
            }
        }
    ...
    flow.selectAll("line")
        ...
        .append("line")
        .attr('class', 'line')
        .attr("x1", d => projection([d.long_ans, d.lat_ans])[0])
        .attr("y1", d => projection([d.long_ans, d.lat_ans])[1])
    ...
    circle.selectAll("circle")
        ...
        .attr('r', 40)
        .attr('id', d => 'circle' + d.ans_owner_country);
        // question ?should ans be ques in the last line?
    ...
    var circleMove = subcircle.selectAll("circle")
        ...
        .attr('class', 'circle')
        .attr('r', 3)
        .attr('id', d => 'CircleMouve' + d.ans_owner_country);
